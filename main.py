...
def parse_args():
    parser = argparse.ArgumentParser(description="few-shot cross-domain")

    # data configuration
    parser.add_argument("--data_path", type=str,
                        default="../data/hwu64.csv",
                        help="path to dataset")
    parser.add_argument("--dataset", type=str, default="ca",
                        help="name of the dataset. "
                        "Options: [cb, ca, ch, bc, bc, bh]")

    parser.add_argument("--n_train_class", type=int, default=23,
                        help="number of meta-train classes")
    parser.add_argument("--n_val_class", type=int, default=16,
                        help="number of meta-val classes")
    parser.add_argument("--n_test_class", type=int, default=25,
    ...
    # model options
    parser.add_argument("--embedding", type=str, default="ebd",
    ...
    parser.add_argument("--notqdm", action="store_true", default=False,
                        help="disable tqdm")
    parser.add_argument("--result_path", type=str, default="results_test.json")
...
def main(args):
    ...
    train_data, val_data, test_data = loader.load_dataset_DG(args)

    # initialize model
    model = {}
    model["ebd"] = ebd.get_embedding(args)
    model["clf"], model["adv"] = clf.get_classifier(model["ebd"].ebd_dim, args)
    ...
    if args.mode == "train":
        ...
        best_path = 'saved-runs/ca_10_3/26'
        model['ebd'].load_state_dict(torch.load(best_path + '.ebd'))
        model['clf'].load_state_dict(torch.load(best_path + '.clf'))
        model['adv'].load_state_dict(torch.load(best_path + '.adv'))
    ...
    if args.result_path:
        ...
        name = ['way', 'shot', 'dataset', 'pretrained_model']
